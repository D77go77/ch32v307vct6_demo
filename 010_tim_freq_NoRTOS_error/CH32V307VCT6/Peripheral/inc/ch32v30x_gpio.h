...
typedef enum
{ GPIO_Mode_AIN = 0x0,              //模拟输入模式 (Analog Input)。
  GPIO_Mode_IN_FLOATING = 0x04,     //浮空输入模式 (Floating Input)。
  GPIO_Mode_IPD = 0x28,             //输入模式（带下拉电阻 Input Pull-Down）。
  GPIO_Mode_IPU = 0x48,             //输入模式（带上拉电阻 Input Pull-Up）。
  GPIO_Mode_Out_OD = 0x14,          //开漏输出模式 (Open Drain Output)。
  GPIO_Mode_Out_PP = 0x10,          //推挽输出模式 (Push-Pull Output)。
  GPIO_Mode_AF_OD = 0x1C,           //开漏输出模式 + 复用功能 (Alternate Function Open-Drain)。
  GPIO_Mode_AF_PP = 0x18            //推挽输出模式 + 复用功能 (Alternate Function Push-Pull)。
}GPIOMode_TypeDef;
